...
## 🎤프로젝트 소개
#### 프로젝트명: 개인 맞춤형 플레이리스트 생성 및 재생 서비스
...
##### **기획목표**:
사용자들에게 자신만의 개인 맞춤형 플레이리스트를 만들고 공유할 수 있는 서비스를 제공함으로써 음악 청취 경험을 향상시키고, 다양한 음악을 소개하며 음악 커뮤니티를 형성한다.
사용자가 직접 플레이리스트를 생성하고 원하는 노래를 추가할 수 있도록 함으로써 창의성과 참여도를 높이며, 개인의 취향과 관심사에 맞는 음악을 쉽게 찾을 수 있도록 한다.
유튜브 링크를 통해 해당 노래를 간편하게 들을 수 있도록 하여 사용자의 편의성을 높이고, 다양한 음악을 탐색하는 재미를 제공한다.

##### **제안 배경**:
현대 사회에서 음악은 매우 중요한 역할을 하며, 사람들은 다양한 음악을 즐기고자 한다. 하지만 기존 음악 스트리밍 플랫폼은 사용자의 취향을 완전히 반영하기 어려운 경우가 많다.
이에 따라 개인 맞춤형 플레이리스트를 만들고 공유하는 서비스가 필요하며, 이를 통해 사용자들은 자신의 취향에 맞는 음악을 손쉽게 발견하고 즐길 수 있다.
또한, 사용자가 직접 플레이리스트를 만들고 원하는 노래를 추가할 수 있는 서비스는 창의성을 유발하고 참여도를 높여 음악 청취 경험을 더욱 풍부하게 만들 수 있다.
유튜브 링크를 통해 해당 노래를 들을 수 있는 기능은 사용자의 편의성을 고려하여 제공함으로써, 음악 검색과 청취를 더욱 간편하게 만들어준다.
...
지금까지 사용해본 DB라고는 firebase에서 기본으로 제공해주는 db였는데 새롭게 mysql을 배우고 이를 응용해서 mybatis를 사용해서 프로젝트를 만들어보니 감회가 새로웠다. 새로 배우는 부분이 많아서 이해하기 많이 힘들고 어려웠지만 그래도 포기하지 않고 끝까지 노력해보니 좋은 결과가 나왔던 것 같다. 학교에서 하는 두 번째 프로젝트라 협업 하는 것은 좀 익숙해서 편하게 했던 것 같다. 서로 소통이 잘 되어서 좋았고 편하게 제작했던 것 같다.
이번 프로젝트로 인해 좀 더 발전하게 된 계기가 된 것 같다. 다음 프로젝트가 기대되기도 하고 걱정이 되기도 한다. 이번에 만든 것으로 아쉬웠던 점이라면 좀 더 열심히 공부해서 기능을 몇 개 더 넣어봤다면 어땠을까 하는 생각이다. 다음 프로젝트에 발전된 모습을 보여주겠다.
